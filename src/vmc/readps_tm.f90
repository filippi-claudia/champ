...
! Also eval pot. at 0 and initializes quadrature pts.
!
! Modified by F. Schautz to use fancy file names
...
!
      ...
      do i=1,npotd
        ...
        vpseudo(1,ic,i)=0.d0
        do j=nr,1,-1
!         write(ounit,'(''vps'',f8.4,f12.6,d12.4)') r(j),vpseudo(j,ic,i)/2,vpseudo(j,ic,i)/2+zion
...
! small radii pot(r)=ce1+ce2*r+ce3*r**2+ce4*r**3+ce5*r**4
        ...
        if(ipr.gt.1) then
!         write(45,'(''coefficients'',1p10e22.10)') (ce(iff),iff=1,NCOEF)
          ...
          do ir=1,10
            ...
            enddo
!             if(ir.eq.1) vpseudo(ir,ic,i)=val
...
! get second derivative for spline fit
        ...
        if(ipr.ge.1) then
          ...
          do j=1,jmax
            write(ipot_io,*) &
!    &      r(j),vpseudo(j,ic,i),d2pot(j,ic,i),-znuc(ic)/r(j),-2*znuc(ic)/r(j)**3
...
!     write(20,'(i6,f18.14,'' nr_ps(ict),arg(ict)'')') nr,arg(ic)
!     do 195 ir=1,nr
! 195   write(20,'(2d20.12)') r(ir),vpseudo(ir,ic,lpot(ic))
...
! compute tm-pseudopotential for electron iel
...
! get spline_fit at r of TM potential for center ic and angular momentum l
! stored on shifted exponential grid
