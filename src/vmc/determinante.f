...
c-----------------------------------------------------------------------
      ...
      if (.not. allocated(ymat_tmp)) then
        ...
        allocate(ymat_tmp(norb_tot,nelec), source=0.0_dp)
      endif
...
c All quantities saved (old) avaliable
      if(iflag_move.eq.1) then
        ...
        if(iguiding.eq.0) then
          ...
          call multideterminante_grad(iel,dorb_tmp,detratio,slmi(1,iab),aa(1,1,iab),ymat(1,1,iab,1),vd)
          ...
          do i=1,nstates
            ...
            call multideterminante_grad(iel,dorb_tmp,detratio,slmi(1,iab),aa(1,1,iab),ymat(1,1,iab,istate),vd_s)
...
c Within single-electron move - quantities of electron iel not saved
       elseif(iflag_move.eq.0) then
        ...
        if(iguiding.eq.0) then
          ...
          call multideterminante_grad(iel,dorbn,detratio,slmin,aan,ymatn,vd)
          ...
          do i=1,nstates
            ...
            call multideterminante_grad(iel,dorbn,detratio,slmin,aan,ymatn(1,1,istate),vd_s)
...
c iel has same spin as electron moved
        if(iflag_move.eq.2) then
          ...
          call multideterminante_grad(iel,dorb_tmp,detratio,slmin,aan,ymatn,vd)
...
c iel has different spin than the electron moved
          ...
          call multideterminante_grad(iel,dorb_tmp,detratio,slmi(1,iab),aa(1,1,iab),ymat_tmp(1,1),vd)
        endif
