     ...
     &,icxdet(MDET*MDETCSFX),iadet(MDET),ibdet(MDET),ncsf,nstates
      ...
      dimension energy_old(MSTATES), energy_err_old(MSTATES), i_deltap(MSTATES), energy_davidson(5,MSTATES) 
          ...
          if(miter.eq.1 .and. iter.gt.1) then          
            ...
            if(iqmc_check.lt.5) then             
              do istate=1,nstates
                ...
                if(diff.ge.5*energy_err_old(istate))then
                  i_deltap(istate)=i_deltap(istate)+1
                  istate0=(istate-1)*nparmci+1
                  call change_ci(deltap_more(istate0,i_deltap(istate)),istate)

                  write(6,'(''STATE, N OVERLAP, ENRGY OLD, ENERGY NEW, 5*ERR '',2i3,3f12.5)') 
     &            istate,i_deltap(istate),energy_old(istate),energy_all(istate),5*energy_err_old(istate)
                  iqmc_again=1
