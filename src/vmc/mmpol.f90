...
! Modified by Riccardo and Claudia
!...........................................................
!     read data for MM-POL calculations
!     computes nuclei-charges  interactions (penu_q)
!     computes nuclei-dipoles  interactions (penu_dp)
!     computes charges-charges interactions (peqq)
!     computes charges-dipoles interactions (peq_dp)
!...........................................................
...
! isites_mmpol is always set to zero in read_input
      if(isites_mmpol.eq.0) then

! Read site positions, fixed charges, and polarizabilities
...
! Compute site-site distances and screening functions, and charge-charge interaction
      call mmpol_compute_peq_q

! Compute nuclei-charge interaction
      call mmpol_compute_penu_q

! Compute dipole-dipole interaction and self-energy
      call mmpol_compute_pedp_dp

! Compute nuclei-dipole interaction
      call mmpol_compute_penu_dp

! Compute nuclei electric field and nuclei-dipole interaction
      call mmpol_field_nu

! Compute charge electric field and charge-dipole interaction
      call mmpol_field_q

! Compute Ainv of mu= Ainv E and electric field due to nuclei and MM charges
...
!............................................................
!     compute distances and screening between sites and
!     peqq  (charges-charges interaction)
!............................................................
      ...
      sixth=1.d0/6.d0
! Compute site-site distances (also charge-charge)
      do k=1,nchmm-1
        do l=k+1,nchmm
...
! Compute charge-charge interaction
      peqq=0.0d0
      do i=1,nchmm-1
        do j=i+1,nchmm
...
!............................................................
!     compute u_self and u_dd (self and dipole-dipole interaction)
!............................................................
      use mmpol_cntrl, only: immpol
      use mmpol_dipol, only: alfa,dipo
...
! Riccardo
! Compute self-energy and dipole-dipole interaction
      u_self=0.d0
      do i=1,nchmm
        dipo_mod=0.d0
        do j=1,3
...
!............................................................
!     compute penu_dp (nuclei-induced dipoles on MM sites interaction)
!............................................................
      use contrl_file, only: ounit
      use mmpol_cntrl, only: immpol
...
!............................................................
!     compute penu_q  (nuclei-charges interaction)
!............................................................
      use mmpol_cntrl, only: immpol
      use mmpol_parms, only: chmm,nchmm,x_mmpol
...
!     ***************************************************************
!     contribution inverse A matrix to compute dipoles as mu=Ainv E
!     ***************************************************************
...
!
!............................................................
!     The matrix ah_pol is computed and inverted
!     (3*nchmm)*(3*nchmm)
!............................................................
...
!
      if(immpol.eq.0) return
!..............................................................
!     enk_pol=nuclear field on MM sites
!..............................................................
      do k=1,nchmm
        enk_pol(1,k)=0.0d0
        enk_pol(2,k)=0.0d0
...
!............................................................
!    computes electric field due to fixed MM charges on MM sites
!    and potential interaction MM charges-dipoles (peq_dp)
!............................................................
      use contrl_file, only: ounit
      use mmpol_cntrl, only: immpol
      use mmpol_dipol, only: dipo
...
! Compute MM charge electric field
      do k=1,nchmm
        eqk_pol(1,k)=0.0d0
        eqk_pol(2,k)=0.0d0
        eqk_pol(3,k)=0.0d0
...
! Compute MM charge-dipole interaction
      peq_dp=0.0d0
      do k=1,nchmm
        do i=1,3
...
!     ***************************************************************
!     For the accepted configuration, the electronic field is computed
!     on MM sites  eek_pol(1,k),eek_pol(2,k),eek_pol(3,k)
!     ***************************************************************
      use mmpol_cntrl, only: immpol
      ...
      do k=1,nchmm
        ...
        do i=1,nelec
          ...
          rr3=rr1*rr2
!..............................................................
!  correction for collision between e- and fixed charges
!..............................................................
...
!......................................................
!       Calculate e-/dipoles interactions (MM_POL)
!                 e-/charges interactions (MM_POL)
!       and adds nuclei-dipoles interactions and
!                nuclei-charges interactions and
!......................................................
!     H(environment) is defined.
!     It includes both H(QM/MM) and H(MM).
!     peQMdp and peQMq are therefore mixed terms
!......................................................
      use contrl_file, only: ounit
...
! Written by Amovilli-Floris
!......................................................
!       Calculate e-/dipoles interactions
!......................................................
      use mmpol_dipol, only: dipo
      use mmpol_fdc, only: rcolm
      use mmpol_parms, only: chmm,nchmm,x_mmpol
      ...
      do j=1,nchmm
        ...
        dpdr=xx*dipo(1,j)+ yy*dipo(2,j)+ zz*dipo(3,j)
!......................................................
!    corrections for collisions electrons-dipoles
!    corrections for collisions electrons-charges
!......................................................
...
!......................................................
!     interaction with fixed charges
!......................................................
...
! u_ef = -mu*E + U_self + U_dd
      ...
      enddo
!...................................................................
!     write in 'efield' the average of solute electrons field
!     write in 'E0' the total static field
!     overwrite dipoles
!...................................................................
      write(56,*)'nchmm'
      write(56,*) nchmm
