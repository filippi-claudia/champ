...
!     if(ns_current.ne.iorbsample) return
! ns_current reset in optorb_sum
...
! nreduced has to be set since it will only be known for non-continuation runs
      nreduced=mreduced
      matdim=nreduced*(nreduced+1)/2
      if(iapprox.gt.0) matdim=nreduced
...
!     if(iorbsample.ne.1) then
!       passes=orb_wcum(istate)
!       passesi=1/passes
!       eave=orb_ecum(istate)*passesi
!     endif
...
! Formulas for exact orbital hessian not implemented
...
! Hamiltonian on semi-orthogonal basis
        idx=0
        do i=1,nreduced
          s(i+ish,1)=0
          s(1,i+ish)=0
...
!         write(ounit,*) 'H',wts,eoav(i)-eave*oav(i),orb_ho_cum(i,istate)*passesi-eave*oav(i)
          i0=1
...
! Perturbative method
       elseif(method.eq.'perturbative') then
        ...
        if(iuse_orbeigv.eq.0) then
! Formulas for exact orbital perturbative not implemented
...
! Approximations on matrix elements
      if(method.eq.'linear') then
       ...
       elseif(method.eq.'perturbative') then
! Approximation: diagonal perturbative approach
        if(iapprox.gt.0) then
          do i=1,nreduced
            do j=1,i-1
              s(j,i)=0
...
!-----------------------------------------------------------------------
      ...
      ne(1)=nup
      ne(2)=nelec
! orbital indices in determinants of trial wave function
      ndetorb=0
