...
! job where it left off
      ...
      if(.not.wid) then
        call mpi_send(nwalk,1,mpi_integer,0,1,MPI_COMM_WORLD,irequest,ierr)
        call mpi_send(xold_dmc,3*nelec*nwalk,mpi_double_precision,0,2,MPI_COMM_WORLD,irequest,ierr)
        call mpi_send(wt,nwalk,mpi_double_precision,0,3,MPI_COMM_WORLD,irequest,ierr)
        call mpi_send(ff(0),nfprod,mpi_double_precision,0,4,MPI_COMM_WORLD,irequest,ierr)
        call mpi_send(fprod,1,mpi_double_precision,0,5,MPI_COMM_WORLD,irequest,ierr)
        call mpi_send(fratio,MWALK*nforce,mpi_double_precision,0,6,MPI_COMM_WORLD,irequest,ierr)
        call mpi_send(eigv,1,mpi_double_precision,0,7,MPI_COMM_WORLD,irequest,ierr)
        call mpi_send(eest,1,mpi_double_precision,0,8,MPI_COMM_WORLD,irequest,ierr)
        call mpi_send(wdsumo,1,mpi_double_precision,0,9,MPI_COMM_WORLD,irequest,ierr)
        call mpi_send(iage,nwalk,mpi_integer,0,10,MPI_COMM_WORLD,irequest,ierr)
        call mpi_send(ioldest,1,mpi_integer,0,11,MPI_COMM_WORLD,irequest,ierr)
        call mpi_send(ioldestmx,1,mpi_integer,0,12,MPI_COMM_WORLD,irequest,ierr)
        call mpi_send(xq,nquad,mpi_double_precision,0,13,MPI_COMM_WORLD,irequest,ierr)
        call mpi_send(yq,nquad,mpi_double_precision,0,14,MPI_COMM_WORLD,irequest,ierr)
        call mpi_send(zq,nquad,mpi_double_precision,0,15,MPI_COMM_WORLD,irequest,ierr)
...
!    &    ,(((wthist(i,l,j),i=1,nwalk),l=0,nwprod-1),j=1,nforce)
      ...
      write(10) (wgcum(i),egcum(i),pecum_dmc(i),tpbcum_dmc(i), &
                 wgcm2(i),egcm2(i),pecm2_dmc(i),tpbcm2_dmc(i),taucum(i),i=1,nforce)
