...
! Update <Oi>,<Ei>,<dEi>,<Oi*Ej>
! Correlation matrix <Oi*Oj> is computed in ci_sum
...
! Compute the gradient (also for first coefficient)
      do i=1,nciterm
...
! Compute the hessian (also for first coefficient)
        idx=0
        do i=1,nciterm
          do j=1,i
            idx=idx+1
...
! h_0,0, h_0,ci, h_ci,0, s_0,ci, s_ci,0
          h_ci(1,1)=etot
          s_ci(1,1)=1
          do j=1,nciterm
...
! iciprt 0 no printout
!         1 each iteration full printout
!        >1 after iciprt iterations reduced printout
!        -1 force printout
