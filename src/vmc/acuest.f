      ...
      do ifr=1,nforce
         do istate=1,nstates
            ...
            if(ifr.eq.1) then
               ...
               fcum(istate,ifr)=fcum(istate,ifr)+wsum(istate,1)
     &              *(enow(istate,ifr)-esum(istate,1)/wsum(istate,1))
               fcm2(istate,ifr)=fcm2(istate,ifr)+wsum(istate,1)
     &              *(enow(istate,ifr)-esum(istate,1)/wsum(istate,1))**2
            endif
         enddo
      enddo
...
c     zero out xsum variables for metrop
      ...
      do istate=1,nstates
         do ifr=1,nforce
            esum(istate,ifr)=0.0d0
            wsum(istate,ifr)=0.0d0
         enddo
         pesum(istate)=0.0d0
         tpbsum(istate)=0.0d0
         tjfsum(istate)=0.0d0
...
c     zero out estimators
      ...
      do ifr=1,nforce
         do istate=1,nstates
            ecum(istate,ifr)=0.0d0
            ecm2(istate,ifr)=0.0d0
            wcum(istate,ifr)=0.0d0
            esum(istate,ifr)=0.0d0
            wsum(istate,ifr)=0.0d0
            fcum(istate,ifr)=0.0d0
            fcm2(istate,ifr)=0.0d0
         enddo
      enddo
...
c     rewrite psi2o if you are sampling guiding
      ...
      if(node_cutoff.gt.0) then
         ...
         if(ipr.gt.1) then
            ...
            do i=1,nelec
               write(6,'(''vd'',3e20.10)') (vold(k,i),k=1,3)
            enddo
         endif
      endif
