...
! No dump/restart if optimizing wave function
      vmc_irstar=0
      vmc_idump=0
...
! CI step for state average of multiple states (optimal CI for input Jastrow and LCAO)
...
! For multiple states, this should never happen as you have checked the energy in the CI step and
! the CI step is unlikely to go wrong (unless the CI run is too short)
...
!     write(ounit,'(/,''change in parameters 1'')')
!     write(ounit,'(''-x='',9f15.9)') (-grad(i),i=1,nparm)
...
!       write(ounit,'(/,''change in parameters '',i1)') iadiag
!       write(ounit,'(''-x='',9f15.9)') (-grad(i),i=1,nparm)
...
! Test run for adiag_1,2,3 with correlated sampling
       nblk_sav=vmc_nblk
       vmc_nblk=max(2,vmc_nblk/2)
!      vmc_nblk=max(2,vmc_nblk/10)
...
! Check if something is very wrong in correlated sampling run
       denergy_min=1.d+99
       denergy_max=0
       do k=1,3
...
!      if(de_worse2.gt.3*de_worse_err2.or.de_worse3.gt.3*de_worse_err3) then
       if(de_worse3.gt.3*de_worse_err3) then
!        write(ounit,'(/,''energy_adiag2_3 is much worse than old energy'')')
...
!      write(ounit,'(/,''Optimal change in parameters'')')
!      write(ounit,'(''-x='',9f15.9)') (-grad(i),i=1,nparm)
...
! CI step for state average of multiple states
      if(ioptci.ne.0.and.nstates.gt.1.and.(ioptorb+ioptjas.gt.0)) then
      800   ioptjas=0
        ioptorb=0
        ioptci=ioptci_sav
...
! add_diag used to generate current Jastrow+orbitals was divided by 10 before the end of the loop 900
!           add_diag(1)=20*add_diag_sav
...
! Jastrow and orbital parameters give worse energy
            ioptjas=ioptjas_sav
            ioptorb=ioptorb_sav
            ioptci=0
...
! save orb and jastrow
        ...
        call set_nparms
! endif CI step for multiple states
      endif
...
! Check if energies for 3 different values of a_diag are less than the tolerance
!     energy_min= 1.d99
!     energy_max=-1.d99
!     do 5 k=1,3
!       energy_min=min(energy_min,energy(k))
!   5   energy_max=max(energy_max,energy(k))
!     e_diff=energy_max-energy_min
!     write(ounit,'(''iter,e_diff='',i4,d12.4)') iter,e_diff
!
!     dforce2=force_err(2)-energy_tol
!     dforce3=force_err(2)-energy_tol
!     if(e_diff.lt.energy_tol.and.dforce2.lt.0.and.dforce3.lt.0) then
!       nblk_new=nblk*max(1.d0,(energy_err_sav/energy_tol)**2)
!       nblk_new=min(nblk_new,nblk_max)
!       if(nblk_new.gt.nblk) then
!         increase_nblk=0
!         nblk=nblk_new
!         write(ounit,'(''nblk reset to'',i8,9d12.4)') nblk,energy_err(1),energy_tol
!       endif
!       write(ounit,'(''energy differences for different add_diag converged to'',d12.4)') energy_tol
!       goto 950
!     endif
...
!     common /gradhess_orb/ grad_orb(norbterm),h_orb(MXMATDIM),s_orb(MXMATDIM)

! Note: we do not vary the first (i0) CI coefficient unless full CI
...
!      do 111 i=1,nparmj+1
! 111    write(ounit,'(''h1= '',1000d12.5)') (h(i,j),j=1,nparmj+1)
!      do 112 i=1,nparmj+1
! 112    write(ounit,'(''h1= '',1000d12.5)') (s(i,j),j=1,nparmj+1)
...
!      write(ounit,'(''h2 shift ='',i4)') ishift
!      do 121 i=1,nciterm-i0
! 121    write(ounit,'(''h2= '',1000f12.5)') (h_ci(i+i0+is,j+i0+is),j=1,nciterm-i0)

! Jastrow-CI Hamiltonian
       do j=1,nciterm-i0
         do i=1,nparmj
...
!      do 131 i=1,nparmj
!        write(ounit,'(''h3= '',1000f12.5)') (h_mix_jas_ci(i,j+i0),j=1,nciterm-i0)
! 131    write(ounit,'(''h3= '',1000f12.5)') (h_mix_jas_ci(i+nparmj,j+i0),j=1,nciterm-i0)
...
!      do 140 i=1,nreduced
!        ik=i*(nreduced+1)
!        h(ishift+i,1)=h_orb(i+1)
!        h(1,ishift+i)=h_orb(ik+1)
!        s(ishift+i,1)=s_orb(i+1)
! 140    s(1,ishift+i)=s_orb(ik+1)

! ORB Hamiltonian
!     do 150 j=1,nreduced
!       jk=j*(nreduced+1)
!        do 150 i=1,nreduced
!          h(ishift+i,ishift+j)=h_orb(i+1+jk)
! 150      s(ishift+i,ishift+j)=s_orb(i+1+jk)

! Jastrow-ORB Hamiltonian
       do j=1,nreduced
         do i=1,nparmj
...
!      do 185 i=1,nparm+1
! 185    write(ounit,'(''s= '',1000d12.5)') (s(i,j),j=1,nparm+1)
