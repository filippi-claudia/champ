...
contains
    subroutine allocate_cuspmat()
        use vmc_mod, only: neqsx
        if (.not. allocated(cm)) allocate (cm(neqsx, neqsx), source=0.0_dp)
        if (.not. allocated(iwc3)) allocate (iwc3(neqsx), source=0)
...
contains
    subroutine allocate_cuspmat4()
        use vmc_mod, only: neqsx, mterms
        if (.not. allocated(d)) allocate (d(neqsx, mterms), source=0.0_dp)
        if (.not. allocated(iwc4)) allocate (iwc4(neqsx), source=0)
...
contains
    subroutine allocate_phifun()
        ...
        if (.not. allocated(dphin)) allocate (dphin(3, nbasis, nelec), source=0.0_dp)
