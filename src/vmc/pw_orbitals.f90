...
!     write(ounit,'(''cos_g,sin_g,dcos_g,dsin_g,ddcos_g,ddsin_g='',30f9.4)')
!    &cos_g(1,1),sin_g(1,1),(dcos_g(k,1,1),k=1,3),(dsin_g(k,1,1),k=1,3),ddcos_g(1,1),ddsin_g(1,1)
!     write(ounit,'(''cos_g,sin_g,dcos_g,dsin_g,ddcos_g,ddsin_g='',30f9.4)')
!    &cos_g(1,2),sin_g(1,2),(dcos_g(k,1,2),k=1,3),(dsin_g(k,1,2),k=1,3),ddcos_g(1,2),ddsin_g(1,2)
...
!             write(ounit,'(''iel,ig,jorb,cos_rp,cos_g(ig),c_rp(iv,jorb)'',3i5,9d12.4)')
...
!           write(ounit,'(''dcos_k(k,ikvec),dsin_k(k,ikvec),dcos_rp(k),dsin_rm(k),dsin_im(k),dcos_ip(k)'',30f9.5)')
!    &(dcos_k(k,ikvec),k=1,3),(dsin_k(k,ikvec),k=1,3),(dcos_rp(k),k=1,3),(dsin_rm(k),k=1,3),(dsin_im(k),k=1,3),(dcos_ip(k),k=1,3
!    &)
...
!      write(ounit,'(''ddcos_k(ikvec),ddsin_k(ikvec),cos_k(ikvec),ddcos_rp,sin_k(ikvec),ddsin_rm='',9f9.4)')
!    &ddcos_k(ikvec),ddsin_k(ikvec),cos_k(ikvec),ddcos_rp,sin_k(ikvec),ddsin_rm

!           write(ounit,'(''orb'',2i5,9d12.4)') iel,iorb,orb(iel,iorb),(dorb(iorb,iel,k),k=1,3)
            do k=1,3
...
!           write(ounit,'(''orb2'',2i5,9d12.4)') iel,iorb,orb(iel,iorb),(dorb(iorb,iel,k),k=1,3)
            do k=1,3
              ddorb(iorb,iel)=ddorb(iorb,iel) &
...
! Calculate pw orbitals, gradient and laplacian for electron iel.
! isortg could be used to map g-vectors from iv to ig and
! isortk could be used to map k-vectors.
! At present it is assumed that both g- and k-vectors are in the correct order.
...
!           write(ounit,'(''orb'',2i5,9d12.4)') iel,iorb,orb(iorb),(dorb(iorb,k),k=1,3)
            do k=1,3
...
!           if(k_inv(ikvec).eq.1) goto 130
            ...
            ddorb(iorb)=ddcos_k(ikvec)*(c_ip(1,jorb)+sin_rm+cos_ip) &
                       ...
                       +cos_k(ikvec)*(ddsin_rm+ddcos_ip) &
                       +sin_k(ikvec)*(ddcos_rp-ddsin_im)
!           write(ounit,'(''orb2'',2i5,9d12.4)') iel,iorb,orb(iorb),(dorb(iorb,k),k=1,3)
            do k=1,3
              ddorb(iorb)=ddorb(iorb) &
                         +2*(dcos_k(k,ikvec)*(dsin_rm(k)+dcos_ip(k)) &
                            +dsin_k(k,ikvec)*(dcos_rp(k)-dsin_im(k)))
