...
! d2j = d_j lapl(ln J) = d_j (lapl(J)/J) - 2 d_j (grad(J)/J) * grad(J)/J
        do j=1,nwftypejas
...
! Compute hessian (symmetrized dj_de term)

! Hessian h = hess1 + hess2 + hess3
! hess1=S_h, hess2=2*G, hess3=terms depending on d2j
      do i=1,nparmj
        do j=1,i
...
! Compute ratio for reweighted expression of the hessian
      botsum_j=0
      topsum_j=0
      do i=1,nparmj
        do j=1,i
...
! Reduced fluctuation hessian = ratio*hess1 + hess3 (ratio, ibeta=-1)
      do i=1,nparmj
        do j=1,i
...
! Compute <dj H dj>/<psi|psi> and <dj dj>/<psi|psi>

! Hamiltonian h = <dj H dj>/<psi|psi>
