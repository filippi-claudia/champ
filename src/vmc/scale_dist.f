...
c     Calculate asymptotic value of A and B terms
      ...
      do istate=1,nstates
         do it=1,nctype
            asymp_jasa(it,istate)=a4(1,it,istate,1)
     &           *asymp_r/(1+a4(2,it,istate,1)*asymp_r)
            do iord=2,norda
               asymp_jasa(it,istate)=asymp_jasa(it,istate)
     &              +a4(iord+1,it,istate,1)*asymp_r**iord
            enddo
         enddo
         ...
         if(ijas.eq.4) then
            do i=1,2
               ...
               asymp_jasb(i,istate)=sspinn*b(1,isp,istate,1)
     &              *asymp_r/(1+b(2,isp,istate,1)*asymp_r)
               do iord=2,nordb
                  asymp_jasb(i,istate)=asymp_jasb(i,istate)
     &                 +b(iord+1,isp,istate,1)*asymp_r**iord
               enddo
            enddo
         elseif(ijas.eq.5) then
            do i=1,2
               ...
               asymp_jasb(i,istate)=b(1,isp,istate,1)
     &              *asymp_r/(1+b(2,isp,istate,1)*asymp_r)
               do iord=2,nordb
                  asymp_jasb(i,istate)=asymp_jasb(i,istate)
     &                 +b(iord+1,isp,istate,1)*asymp_r**iord
               enddo
               asymp_jasb(i,istate)=sspinn*asymp_jasb(i,istate)
            enddo
         endif
         if((ijas.eq.4.or.ijas.eq.5).and.ipr.gt.1) then
            write(6,'(''STATE'',I1.1)') istate
            write(6,'(''asymp_r='',f10.6)') asymp_r
            write(6,'(''asympa='',10f10.6)') (asymp_jasa(it,istate),it=1,nctype)
            write(6,'(''asympb='',10f10.6)') (asymp_jasb(i,istate),i=1,2)
         endif
      enddo
