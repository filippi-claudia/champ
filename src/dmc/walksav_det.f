...
c Written by Claudia Filippi
      ...
      if(.not.allocated(aaw)) allocate(aaw(nelec,norb_tot,mwalk,2), source=0.0_dp)
      if(.not.allocated(wfmatw)) allocate(wfmatw(MEXCIT**2,ndet,mwalk,2), source=0.0_dp)
      if(.not.allocated(ymatw)) allocate(ymatw(norb_tot,nelec,mwalk,2,MSTATES), source=0.0_dp)
      if(.not.allocated(orbw)) allocate(orbw(nelec,norb_tot,mwalk), source=0.0_dp)
      if(.not.allocated(dorbw)) allocate(dorbw(3,nelec,norb_tot,mwalk), source=0.0_dp)

      if(.not.allocated(krefw)) allocate(krefw(mwalk), source=0)
      if(.not.allocated(slmuiw)) allocate(slmuiw(nmat_dim,mwalk), source=0.0_dp)
      if(.not.allocated(slmdiw)) allocate(slmdiw(nmat_dim,mwalk), source=0.0_dp)
      if(.not.allocated(fpuw)) allocate(fpuw(3, nmat_dim,mwalk), source=0.0_dp)
      if(.not.allocated(fpdw)) allocate(fpdw(3, nmat_dim,mwalk), source=0.0_dp)
      if(.not.allocated(fppuw)) allocate(fppuw(nmat_dim,mwalk), source=0.0_dp)
      if(.not.allocated(fppdw)) allocate(fppdw(nmat_dim,mwalk), source=0.0_dp)
      if(.not.allocated(ddxw)) allocate(ddxw(3, nelec,mwalk), source=0.0_dp)
      if(.not.allocated(d2dx2w)) allocate(d2dx2w(nelec,mwalk), source=0.0_dp)
      if(.not.allocated(detuw)) allocate(detuw(ndet,mwalk), source=0.0_dp)
      if(.not.allocated(detdw)) allocate(detdw(ndet,mwalk), source=0.0_dp)
