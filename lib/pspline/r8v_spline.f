...
!Set first and second BC values
     ...
     &         -(f(1,2)-f(1,1))/(x(2)-x(1)))/(x(3)-x(1))
      ...
      ELSEIF(k_bcn.eq.6) THEN
        bn=2.0d0*((f(1,n)-f(1,n-1))/(x(n)-x(n-1))
...
!Compute polynomial coefficients
        DO i=1,n-1
          f(2,i)=
     >        (f(1,i+1)-f(1,i))/f(4,i)-f(4,i)*(f(3,i+1)+2.0d0*f(3,i))
