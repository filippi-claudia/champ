      subroutine deriv_jastrow4(x,v,d2,value,istate)
c     Written by Cyrus Umrigar and Claudia Filippi
      use vmc_mod, only: MELEC, MCENT
      use vmc_mod, only: MORDJ, MMAT_DIM2
      use vmc_mod, only: NEQSX, MTERMS
      use atom, only: iwctype, nctype, ncent
      use const, only: nelec
      use derivjas, only: d2g, g, go, gvalue
      use elec, only: nup
      use ijasnonlin, only: d1d2a, d1d2b, d2d2a, d2d2b
      use jaso, only: d2ijo, d2o, fijo, fjo, fso, fsumo
      use jaspar, only: sspinn
      use jaspar3, only: a, b, c
      use jaspar4, only: a4, norda, nordb, nordc
      use jaspar6, only: asymp_jasa, asymp_jasb, asymp_r
      use jaspar6, only: cutjas
      use jaspointer, only: npoint, npointa
      use optwf_contrl, only: ioptjas
      use optwf_nparmj, only: nparma, nparmb
      use optwf_parms, only: nparmj
      use optwf_wjas, only: iwjasa, iwjasb, iwjasc
      use wfsec, only: iwf
      use bparm, only: nocuspb, nspin2b
      use contr2, only: ijas
      use contr2, only: isc
      use vardep, only: cdep, iwdepend, nvdepend
      use distance_mod, only: rshift, r_en, rvec_en
      use force_analy, only: iforce_analy
      use cuspmat4, only: d, iwc4, nterms
      use distance_mod, only: rshift, r_en, rvec_en, r_ee, rvec_ee

      implicit real*8(a-h,o-z)

      parameter (zero=0.0d0,one=1.0d0,two=2.0d0)
      parameter (half=0.5d0,eps=1.0d-12)

      dimension x(3,*),v(3,*)
      dimension uu(-2:MORDJ),ss(-2:MORDJ),tt(-2:MORDJ),rri(-2:MORDJ)
     &     ,rrj(-2:MORDJ)

      iparma=nparma(1)
      do it=2,nctype
         iparma=iparma+nparma(it)
      enddo

      fsum=0.0d0
      d2=0.0d0

      do i=1,nelec
         v(1,i)=0.0d0
         v(2,i)=0.0d0
         v(3,i)=0.0d0
      enddo

      do i=-2,-1
         uu(i)=0.0d0
         ss(i)=0.0d0
         tt(i)=0.0d0
         rri(i)=0.0d0
         rrj(i)=0.0d0
      enddo

      uu(0)=1
      ss(0)=2
      tt(0)=1
      rri(0)=1
      rrj(0)=1

      do iparm=1,nparmj
         gvalue(iparm,istate)=0.0d0
         d2g(iparm,istate)=0.0d0
         do i=1,nelec
            g(1,i,iparm,istate)=0.0d0
            g(2,i,iparm,istate)=0.0d0
            g(3,i,iparm,istate)=0.0d0
         enddo
      enddo

      do isb=1,nspin2b
         d1d2b(isb)=0.0d0
         d2d2b(isb)=0.0d0
      enddo

      do it=1,nctype
         d1d2a(it)=0.0d0
         d2d2a(it)=0.0d0
      enddo

      if (nelec.lt.2) goto 65

c     e-e and e-e-n terms
      ij=0
      do 60 i=2,nelec
         im1=i-1
         do 60 j=1,im1
            ij=ij+1

            fso(i,j,istate)=0.0d0
            d2ijo(i,j,istate)=0.0d0
            do k=1,3
               fijo(k,i,j,istate)=0.0d0
               fijo(k,j,i,istate)=0.0d0
            enddo
            do iparm=1,nparmj
               go(i,j,iparm,istate)=0.0d0
            enddo

            sspinn=1
            isb=1
            ipar=0
            if(i.le.nup .or. j.gt.nup) then
               if(nspin2b.eq.2) then
                  isb=2
               elseif(nocuspb.eq.0) then
                  sspinn=half
               endif
               ipar=1
            endif

            rij=r_ee(ij)

            call scale_dist2(rij,uu(1),dd1,dd2,1)

c     Check rij after scaling because uu(1) used in e-e-n terms too
            if(rij.gt.cutjas) goto 30

            top=sspinn*b(1,isb,istate,iwf)*uu(1)
            topu=sspinn*b(1,isb,istate,iwf)
            topuu=0.0d0

            bot=1+b(2,isb,istate,iwf)*uu(1)
            botu=b(2,isb,istate,iwf)
            botuu=0.0d0
            bot2=bot*bot

            fee=top/bot-asymp_jasb(ipar+1,istate)
            feeu=topu/bot-botu*top/bot2
            feeuu=topuu-(botuu*top+2*botu*topu)/bot+2*botu**2*top/bot2
            feeuu=feeuu/bot

            do iord=2,nordb
               uu(iord)=uu(1)*uu(iord-1)
               fee=fee+b(iord+1,isb,istate,iwf)*uu(iord)
               feeu=feeu+b(iord+1,isb,istate,iwf)*iord*uu(iord-1)
               feeuu=feeuu+b(iord+1,isb,istate,iwf)*iord*(iord-1)*uu(iord-2)
            enddo

            feeuu=feeuu*dd1*dd1+feeu*dd2
            feeu=feeu*dd1/rij

            fso(i,j,istate)=fso(i,j,istate)+fee
            do k=1,3
               fijo(k,i,j,istate)=fijo(k,i,j,istate)+feeu*rvec_ee(k,ij)
               fijo(k,j,i,istate)=fijo(k,j,i,istate)-feeu*rvec_ee(k,ij)
            enddo
            d2ijo(i,j,istate)=d2ijo(i,j,istate)+2*(feeuu+2*feeu)
c     derivatives of wave function wrt b(1),b(2) and rest of b(i)
            iparm0=iparma
            if(isb.eq.2) iparm0=iparm0+nparmb(1)
            do jparm=1,nparmb(isb)
               iparm=iparm0+jparm

               if(iwjasb(jparm,isb).eq.1) then
                  top=sspinn*uu(1)
                  topu=sspinn
                  topuu=zero

                  bot=one+b(2,isb,istate,iwf)*uu(1)
                  botu=b(2,isb,istate,iwf)
                  botuu=zero
                  bot2=bot*bot

                  gee=top/bot-sspinn*asymp_r/(1+b(2,isb,istate,iwf)*asymp_r)
                  geeu=topu/bot-botu*top/bot2
                  geeuu=topuu-(botuu*top+2*botu*topu)/bot+2*botu**2*top/bot2
                  geeuu=geeuu/bot

               elseif(iwjasb(jparm,isb).eq.2) then
                  top=-sspinn*b(1,isb,istate,iwf)*uu(1)*uu(1)
                  topu=-sspinn*2*b(1,isb,istate,iwf)*uu(1)
                  topuu=-sspinn*2*b(1,isb,istate,iwf)

                  bot0=one+b(2,isb,istate,iwf)*uu(1)
                  bot=bot0*bot0
                  botu=2*bot0*b(2,isb,istate,iwf)
                  botuu=2*b(2,isb,istate,iwf)*b(2,isb,istate,iwf)
                  bot2=bot*bot

                  gee=top/bot+sspinn*b(1,isb,istate,iwf)*asymp_r**2/(1+b(2,isb,istate,iwf)*asymp_r)**2
                  geeu=topu/bot-botu*top/bot2
                  geeuu=topuu-(botuu*top+2*botu*topu)/bot+2*botu**2*top/bot2
                  geeuu=geeuu/bot

                  if(ioptjas.gt.0) then
                     d2d2b(isb)=d2d2b(isb)-2*top*uu(1)/bot/bot0-2*sspinn
     &                          *b(1,isb,istate,iwf)*asymp_r**3/(1+b(2,isb,istate,iwf)*asymp_r)**3
                     d1d2b(isb)=d1d2b(isb)-sspinn*uu(1)*uu(1)/bot+sspinn
     &                          *asymp_r**2/(1+b(2,isb,istate,iwf)*asymp_r)**2
                  endif
               else
                  iord=iwjasb(jparm,isb)-1
                  if(ijas.eq.4) then
                     gee=uu(iord)-asymp_r**iord
                     geeu=iord*uu(iord-1)
                     geeuu=iord*(iord-1)*uu(iord-2)
                  elseif(ijas.eq.5) then
                     gee=sspinn*(uu(iord)-asymp_r**iord)
                     geeu=sspinn*iord*uu(iord-1)
                     geeuu=sspinn*iord*(iord-1)*uu(iord-2)
                  endif

               endif

               geeuu=geeuu*dd1*dd1+geeu*dd2
               geeu=geeu*dd1/rij

               go(i,j,iparm,istate)=go(i,j,iparm,istate)+gee
               gvalue(iparm,istate)=gvalue(iparm,istate)+gee

               g(1,i,iparm,istate)=g(1,i,iparm,istate) + geeu*rvec_ee(1,ij)
               g(2,i,iparm,istate)=g(2,i,iparm,istate) + geeu*rvec_ee(2,ij)
               g(3,i,iparm,istate)=g(3,i,iparm,istate) + geeu*rvec_ee(3,ij)
               g(1,j,iparm,istate)=g(1,j,iparm,istate) - geeu*rvec_ee(1,ij)
               g(2,j,iparm,istate)=g(2,j,iparm,istate) - geeu*rvec_ee(2,ij)
               g(3,j,iparm,istate)=g(3,j,iparm,istate) - geeu*rvec_ee(3,ij)

               d2g(iparm,istate)=d2g(iparm,istate) + two*(geeuu+two*geeu)

            enddo

c     There are no C terms to order 1.
 30         if(nordc.le.1) goto 58

            if(isc.ge.12) call scale_dist2(rij,uu(1),dd1,dd2,3)
            if(ijas.eq.4.or.ijas.eq.5) then
               call switch_scale2(uu(1),dd1,dd2)
               do iord=2,nordc
                  uu(iord)=uu(1)*uu(iord-1)
               enddo
            endif

            do ic=1,ncent
               it=iwctype(ic)
               ri=r_en(i,ic)
               rj=r_en(j,ic)

               if(ri.gt.cutjas .or. rj.gt.cutjas) cycle
               do k=1,3
                  if(abs(rshift(k,i,ic)-rshift(k,j,ic)).gt.eps) cycle
               enddo

               call scale_dist2(ri,rri(1),dd7,dd9,2)
               call scale_dist2(rj,rrj(1),dd8,dd10,2)

               if(ijas.eq.4.or.ijas.eq.5) then
                  call switch_scale2(rri(1),dd7,dd9)
                  call switch_scale2(rrj(1),dd8,dd10)
               endif

               s=ri+rj
               t=ri-rj
               u2pst=rij*rij+s*t
               u2mst=rij*rij-s*t

               do iord=1,nordc
                  rri(iord)=rri(1)*rri(iord-1)
                  rrj(iord)=rrj(1)*rrj(iord-1)
                  ss(iord)=rri(iord)+rrj(iord)
                  tt(iord)=rri(iord)*rrj(iord)
               enddo

               fc=0.0d0
               fu=0.0d0
               fuu=0.0d0
               fi=0.0d0
               fii=0.0d0
               fj=0.0d0
               fjj=0.0d0
               fui=0.0d0
               fuj=0.0d0
               ll=0.0d0
               jj=1
               jparm=1
               do n=2,nordc
                  do k=n-1,0,-1
                     if(k.eq.0) then
                        l_hi=n-k-2
                     else
                        l_hi=n-k
                     endif
                     do l=l_hi,0,-1
                        m=(n-k-l)/2
                        if(2*m.eq.n-k-l) then
                           ll=ll+1
                           pc=uu(k)*ss(l)*tt(m)
                           pu=k*uu(k-1)*ss(l)*tt(m)
                           puu=k*(k-1)*uu(k-2)*ss(l)*tt(m)
                           ppi=uu(k)
     &                          *((l+m)*rri(l+m-1)*rrj(m)+m*rri(m-1)*rrj(l+m))
                           pii=uu(k)
     &                          *((l+m)*(l+m-1)*rri(l+m-2)*rrj(m)
     &                          +m*(m-1)*rri(m-2)*rrj(l+m))
                           pj=uu(k)
     &                          *((l+m)*rrj(l+m-1)*rri(m)+m*rrj(m-1)*rri(l+m))
                           pjj=uu(k)
     &                          *((l+m)*(l+m-1)*rrj(l+m-2)*rri(m)
     &                          +m*(m-1)*rrj(m-2)*rri(l+m))
                           pui=k*uu(k-1)
     &                          *((l+m)*rri(l+m-1)*rrj(m)+m*rri(m-1)*rrj(l+m))
                           puj=k*uu(k-1)
     &                          *((l+m)*rrj(l+m-1)*rri(m)+m*rrj(m-1)*rri(l+m))

                           fc=fc+c(ll,it,istate,iwf)*pc
                           fu=fu+c(ll,it,istate,iwf)*pu
                           fuu=fuu+c(ll,it,istate,iwf)*puu
                           fi=fi+c(ll,it,istate,iwf)*ppi
                           fii=fii+c(ll,it,istate,iwf)*pii
                           fj=fj+c(ll,it,istate,iwf)*pj
                           fjj=fjj+c(ll,it,istate,iwf)*pjj
                           fui=fui+c(ll,it,istate,iwf)*pui
                           fuj=fuj+c(ll,it,istate,iwf)*puj

                           ideriv=0
                           if(ll.eq.iwjasc(jparm,it)) then
                              ideriv=2
                           else
                              do id=1,2*(nordc-1)
                                 if(ll.eq.iwc4(id)) then
                                    jj=id
                                    if(nvdepend(jj,it).gt.0) ideriv=1
                                 endif
                              enddo
                           endif

                           if(ideriv.gt.0) then
                              gp=pc
                              gu=pu
                              guu=puu
                              gi=ppi
                              gii=pii
                              gj=pj
                              gjj=pjj
                              gui=pui
                              guj=puj
                              guu=guu*dd1*dd1+gu*dd2
                              gu=gu*dd1/rij
                              gui=gui*dd1*dd7
                              guj=guj*dd1*dd8
                              gii=gii*dd7*dd7+gi*dd9
                              gjj=gjj*dd8*dd8+gj*dd10
                              gi=gi*dd7/ri
                              gj=gj*dd8/rj
                              if(ideriv.eq.1) then
                                 do id=1,nvdepend(jj,it)
                                    iparm=npoint(it)+iwdepend(jj,id,it)
                                    cd=cdep(jj,id,it)
                                    go(i,j,iparm,istate)=go(i,j,iparm,istate)+cd*gp
                                    gvalue(iparm,istate)=gvalue(iparm,istate)+cd*gp
                                    g(1,i,iparm,istate)=g(1,i,iparm,istate)+
     &                                  cd*(gi*rvec_en(1,i,ic)+gu*rvec_ee(1,ij))
                                    g(2,i,iparm,istate)=g(2,i,iparm,istate)+
     &                                  cd*(gi*rvec_en(2,i,ic)+gu*rvec_ee(2,ij))
                                    g(3,i,iparm,istate)=g(3,i,iparm,istate)+
     &                                  cd*(gi*rvec_en(3,i,ic)+gu*rvec_ee(3,ij))
                                    g(1,j,iparm,istate)=g(1,j,iparm,istate)+
     &                                  cd*(gj*rvec_en(1,j,ic)-gu*rvec_ee(1,ij))
                                    g(2,j,iparm,istate)=g(2,j,iparm,istate)+
     &                                  cd*(gj*rvec_en(2,j,ic)-gu*rvec_ee(2,ij))
                                    g(3,j,iparm,istate)=g(3,j,iparm,istate)+
     &                                  cd*(gj*rvec_en(3,j,ic)-gu*rvec_ee(3,ij))
                                    d2g(iparm,istate)=d2g(iparm,istate)+cd*(2*(guu + 2*gu)
     &                                   + gui*u2pst/(ri*rij) + guj*u2mst/(rj*rij)
     &                                   + gii + 2*gi + gjj + 2*gj)
                                 enddo
                              elseif(ideriv.eq.2) then
                                 iparm=npoint(it)+jparm
                                 go(i,j,iparm,istate)=go(i,j,iparm,istate)+gp
                                 gvalue(iparm,istate)=gvalue(iparm,istate)+gp
                                 g(1,i,iparm,istate)=g(1,i,iparm,istate)+gi*rvec_en(1,i,ic)
     &                                        +gu*rvec_ee(1,ij)
                                 g(2,i,iparm,istate)=g(2,i,iparm,istate)+gi*rvec_en(2,i,ic)
     &                                        +gu*rvec_ee(2,ij)
                                 g(3,i,iparm,istate)=g(3,i,iparm,istate)+gi*rvec_en(3,i,ic)
     &                                        +gu*rvec_ee(3,ij)
                                 g(1,j,iparm,istate)=g(1,j,iparm,istate)+gj*rvec_en(1,j,ic)
     &                                        -gu*rvec_ee(1,ij)
                                 g(2,j,iparm,istate)=g(2,j,iparm,istate)+gj*rvec_en(2,j,ic)
     &                                        -gu*rvec_ee(2,ij)
                                 g(3,j,iparm,istate)=g(3,j,iparm,istate)+gj*rvec_en(3,j,ic)
     &                                        -gu*rvec_ee(3,ij)
                                 d2g(iparm,istate)=d2g(iparm,istate) + 2*(guu + 2*gu)
     &                                + gui*u2pst/(ri*rij) + guj*u2mst/(rj*rij)
     &                                + gii + 2*gi + gjj + 2*gj
                                 jparm=jparm+1
                              endif
                           endif
                        endif
                     enddo
                  enddo
               enddo

               fuu=fuu*dd1*dd1+fu*dd2
               fu=fu*dd1/rij

               fui=fui*dd1*dd7
               fuj=fuj*dd1*dd8

               fii=fii*dd7*dd7+fi*dd9
               fjj=fjj*dd8*dd8+fj*dd10
               fi=fi*dd7/ri
               fj=fj*dd8/rj

               fso(i,j,istate)=fso(i,j,istate)+fc

               fijo(1,i,j,istate)=fijo(1,i,j,istate)+fi*rvec_en(1,i,ic)+fu*rvec_ee(1,ij)
               fijo(2,i,j,istate)=fijo(2,i,j,istate)+fi*rvec_en(2,i,ic)+fu*rvec_ee(2,ij)
               fijo(3,i,j,istate)=fijo(3,i,j,istate)+fi*rvec_en(3,i,ic)+fu*rvec_ee(3,ij)
               fijo(1,j,i,istate)=fijo(1,j,i,istate)+fj*rvec_en(1,j,ic)-fu*rvec_ee(1,ij)
               fijo(2,j,i,istate)=fijo(2,j,i,istate)+fj*rvec_en(2,j,ic)-fu*rvec_ee(2,ij)
               fijo(3,j,i,istate)=fijo(3,j,i,istate)+fj*rvec_en(3,j,ic)-fu*rvec_ee(3,ij)

               d2ijo(i,j,istate)=d2ijo(i,j,istate) + 2*(fuu + 2*fu) + fui*u2pst/(ri*rij)
     &              + fuj*u2mst/(rj*rij) + fii + 2*fi + fjj + 2*fj

            enddo

 58         fsum=fsum+fso(i,j,istate)
            v(1,i)=v(1,i)+fijo(1,i,j,istate)
            v(2,i)=v(2,i)+fijo(2,i,j,istate)
            v(3,i)=v(3,i)+fijo(3,i,j,istate)
            v(1,j)=v(1,j)+fijo(1,j,i,istate)
            v(2,j)=v(2,j)+fijo(2,j,i,istate)
            v(3,j)=v(3,j)+fijo(3,j,i,istate)
 60   d2=d2+d2ijo(i,j,istate)

c     e-n terms
 65   do i=1,nelec
         fso(i,i,istate)=0.0d0
         fijo(1,i,i,istate)=0.0d0
         fijo(2,i,i,istate)=0.0d0
         fijo(3,i,i,istate)=0.0d0
         d2ijo(i,i,istate)=0.0d0

         do iparm=1,nparmj
            go(i,i,iparm,istate)=zero
         enddo

         do ic=1,ncent
            it=iwctype(ic)
            ri=r_en(i,ic)
            if(ri.gt.cutjas) cycle

            call scale_dist2(ri,rri(1),dd7,dd9,1)

            top=a4(1,it,istate,iwf)*rri(1)
            topi=a4(1,it,istate,iwf)
            topii=0.0d0

            bot=1+a4(2,it,istate,iwf)*rri(1)
            boti=a4(2,it,istate,iwf)
            botii=0.0d0
            bot2=bot*bot

            fen=top/bot-asymp_jasa(it,istate)
            feni=topi/bot-boti*top/bot2
            fenii=topii-(botii*top+2*boti*topi)/bot+2*boti**2*top/bot2
            fenii=fenii/bot

            do iord=2,norda
               rri(iord)=rri(1)*rri(iord-1)
               fen=fen+a4(iord+1,it,istate,iwf)*rri(iord)
               feni=feni+a4(iord+1,it,istate,iwf)*iord*rri(iord-1)
               fenii=fenii+a4(iord+1,it,istate,iwf)*iord*(iord-1)*rri(iord-2)
            enddo

            feni_save=feni
            fenii_save=fenii
            fenii=fenii*dd7*dd7+feni*dd9
            feni=feni*dd7/ri
            fso(i,i,istate)=fso(i,i,istate)+fen
            fijo(1,i,i,istate)=fijo(1,i,i,istate)+feni*rvec_en(1,i,ic)
            fijo(2,i,i,istate)=fijo(2,i,i,istate)+feni*rvec_en(2,i,ic)
            fijo(3,i,i,istate)=fijo(3,i,i,istate)+feni*rvec_en(3,i,ic)
            d2ijo(i,i,istate)=d2ijo(i,i,istate)+fenii+2*feni

            if(iforce_analy.eq.1) then
               call da_jastrow4(iwf,i,ic,it,rvec_en(1,i,ic),ri,rri,feni_save,fenii_save,dd7,dd9)
            endif

            do jparm=1,nparma(it)
               iparm=npointa(it)+jparm

               if(iwjasa(jparm,it).eq.1) then
                  top=rri(1)
                  topi=one
                  topii=zero

                  bot=one+a4(2,it,istate,iwf)*rri(1)
                  boti=a4(2,it,istate,iwf)
                  botii=zero
                  bot2=bot*bot

                  gen=top/bot-asymp_r/(1+a4(2,it,istate,iwf)*asymp_r)
                  geni=topi/bot-boti*top/bot2
                  genii=topii-(botii*top+2*boti*topi)/bot+2*boti**2*top/bot2
                  genii=genii/bot

               elseif(iwjasa(jparm,it).eq.2) then
                  top=-a4(1,it,istate,iwf)*rri(1)*rri(1)
                  topi=-2*a4(1,it,istate,iwf)*rri(1)
                  topii=-2*a4(1,it,istate,iwf)

                  bot0=one+a4(2,it,istate,iwf)*rri(1)
                  bot=bot0*bot0
                  boti=2*bot0*a4(2,it,istate,iwf)
                  botii=2*a4(2,it,istate,iwf)*a4(2,it,istate,iwf)
                  bot2=bot*bot

                  gen=top/bot+a4(1,it,istate,iwf)*asymp_r**2/(1+a4(2,it,istate,iwf)*asymp_r)**2
                  geni=topi/bot-boti*top/bot2
                  genii=topii-(botii*top+2*boti*topi)/bot+2*boti**2*top/bot2
                  genii=genii/bot

                  if(ioptjas.gt.0) then
                     d2d2a(it)=d2d2a(it)-2*top*rri(1)/bot/bot0-2*a4(1,it,istate,iwf)
     &                        *asymp_r**3/(1+a4(2,it,istate,iwf)*asymp_r)**3
                     d1d2a(it)=d1d2a(it)-rri(1)*rri(1)/bot+asymp_r**2/(1+a4(2,it,istate,iwf)*asymp_r)**2
                  endif
               else
                  iord=iwjasa(jparm,it)-1
                  gen=rri(iord)-asymp_r**iord
                  geni=iord*rri(iord-1)
                  genii=iord*(iord-1)*rri(iord-2)

               endif

               genii=genii*dd7*dd7+geni*dd9
               geni=geni*dd7/r_en(i,ic)

               go(i,i,iparm,istate)=go(i,i,iparm,istate)+gen
               gvalue(iparm,istate)=gvalue(iparm,istate)+gen

               g(1,i,iparm,istate)=g(1,i,iparm,istate)+geni*rvec_en(1,i,ic)
               g(2,i,iparm,istate)=g(2,i,iparm,istate)+geni*rvec_en(2,i,ic)
               g(3,i,iparm,istate)=g(3,i,iparm,istate)+geni*rvec_en(3,i,ic)

               d2g(iparm,istate)=d2g(iparm,istate)+genii+two*geni
            enddo
         enddo

         fsum=fsum+fso(i,i,istate)
         v(1,i)=v(1,i)+fijo(1,i,i,istate)
         v(2,i)=v(2,i)+fijo(2,i,i,istate)
         v(3,i)=v(3,i)+fijo(3,i,i,istate)
         d2=d2+d2ijo(i,i,istate)
      enddo

      fsumo(istate)=fsum
      d2o=d2
      do i=1,nelec
         fjo(1,i,istate)=v(1,i)
         fjo(2,i,istate)=v(2,i)
         fjo(3,i,istate)=v(3,i)
      enddo

      value=fsum

      end subroutine
