...
! If we optimize jas, orb, or both, we solve secular super-CI equation on basis of psi_0,dpsi_0 for a total dimension of nparm+1
      is=1
      if(ioptjas.eq.0.and.ioptorb.eq.0) then
! If CI only, we solve secular equation on basis of J*CSF
        is=0
       else
! Save the overlap and hamiltonian
        idx=0
        do i=1,nparm+is
          do j=1,i
            idx=idx+1
...
! use semiorthogonal basis and compute minimum norm in direction orthogonal to psi0
      dmult=1.d0
      scale=.1d0
...
!       add_diag=max(add_diag,1.d-2*abs(eig_min))
       else
!       add_diag=0
...
!     call cpu_time(t)
!     t_sdiag=t
!     write(ounit,*) 'elapsed time for diagonalization:',t_sdiag-t0
...
! I = A^T S A where A_ij= U_ij/sqrt(s_eigval(j))
! s in input contains U -> s is overwritten and contains A
      do i=1,n
        do j=1,n
...
! Compute work_mat=A^T*H*A
      ...
      call cpu_time(t)
!     t_hmod=t
!     write(ounit,*) 'elapsed time to build Hmod:',t_hmod-t_sdiag
...
!     call cpu_time(t)
!     t_eigvec=t
!     write(ounit,*) 'elapsed time to get eigenvectors:',t_eigvec-t_hmdiag
...
!     do i=1,nparm+is
!     write(ounit,'(''h '',1000e25.15)') (h(i,j),j=1,nparm+is)
!     enddo
!     do i=1,nparm+is
!     write(ounit,'(''s '',1000e25.15)') (s(i,j),j=1,nparm+is)
!     enddo
...
! minus sign because variation is subtracted when computing new parameters
        do i=1,nparm
