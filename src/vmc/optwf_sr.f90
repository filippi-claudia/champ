...
6               continue
    ...
    subroutine compute_norm_lin(nparm,deltap)  ! check these modules and rest of subroutine
        ...
        allocate(obs_norm_tot(mobs,MSTATES))
    ...
    subroutine sr(nparm, h_sr, deltap, sr_adiag, sr_eps, i)
        ...
        integer, intent(in) :: nparm
        real(dp), dimension(:), intent(in) :: h_sr
...
! gradient wrt parameters
              ...
              enddo
              write(ounit,'(''max S diagonal element '',t41,d9.2)') smax
...
!            endif
    ...
    subroutine print_gradients(nparm, deltap)

        use mpiconf, only: idtask
        use csfs, only: nstates
        use orb_mat_022, only: ideriv
        use optwf_parms, only: nparmj
        use sr_mat_n, only: h_sr, s_ii_inv, h_sr_penalty
        use sr_mod, only: mparm
        use mstates_mod, only: MSTATES
        use optorb_cblock, only: norbterm
        use contrl_file, only: ounit
        ...
        integer :: j, k, iorbstart, iciend, icistart

        ! stu added
        if (idtask.eq.0) then
          icistart=nparmj+1
          iciend=nparm-norbterm
          iorbstart=iciend+1

          do j=1,nstates
            if (nstates.eq.1) then
              write(ounit,*) "State: ", j
              write(ounit,*) "Jastrow Parameters"
              write(ounit,*) "parm, deltap, full grad, s_ii_inv"
              do k=1,nparmj
                write(ounit,'(i7,3es26.15e3)') k, deltap(k,j), h_sr(k,j), s_ii_inv(k,j)
              enddo
              write(ounit,*) "CI Parameters"
              write(ounit,*) "parm, citerm, deltap, full grad, s_ii_inv"
              do k=icistart,iciend
                write(ounit,'(2i7,3es26.15e3)') k, k-nparmj, deltap(k,j), h_sr(k,j), s_ii_inv(k,j)
              enddo
              write(ounit,*) "Orbital Mixing Parameters"
              write(ounit,*) "parm, orbterm, deltap, full grad, s_ii_inv io, jo"
              do k=iorbstart,nparm
                write(ounit,'(2i7,3es26.15e3,2i7)') k, k-iciend, deltap(k,j), h_sr(k,j), s_ii_inv(k,j), ideriv(1,k-iciend),ideriv(2,k-iciend)
              enddo
            else
              write(ounit,*) "State: ", j
              write(ounit,*) "Jastrow Parameters"
              write(ounit,*) "parm, deltap, full grad, ortho grad, s_ii_inv"
              do k=1,nparmj
                write(ounit,'(i7,4es26.15e3)') k, deltap(k,j), h_sr(k,j), h_sr_penalty(k,j), s_ii_inv(k,j)
              enddo
              write(ounit,*) "CI Parameters"
              write(ounit,*) "parm, citerm, deltap, full grad, ortho grad, s_ii_inv"
              do k=icistart,iciend
                write(ounit,'(2i7,4es26.15e3)') k, k-nparmj, deltap(k,j), h_sr(k,j), h_sr_penalty(k,j), s_ii_inv(k,j)
              enddo
              write(ounit,*) "Orbital Mixing Parameters"
              write(ounit,*) "parm, orbterm, deltap, full grad, ortho grad, s_ii_inv io, jo"
              do k=iorbstart,nparm
                write(ounit,'(2i7,4es26.15e3,2i7)') k, k-iciend, deltap(k,j), h_sr(k,j), h_sr_penalty(k,j), s_ii_inv(k,j),ideriv(1,k-iciend), ideriv(2,k-iciend)
              enddo
            endif
          enddo
        endif
