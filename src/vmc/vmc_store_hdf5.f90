module vmc_store_hdf5_mod
        ...
        if (idtask .ne. 0) then
          ...
          if (nloc .gt. 0) then
            ...
            call mpi_send(wq,nquad,mpi_double_precision,0,5,MPI_COMM_WORLD,ierr)
          endif
        else
          do id=1,nproc-1
             call mpi_recv(x_id,3*nelec,mpi_double_precision,id,1,MPI_COMM_WORLD,istatus,ierr)
             if (nloc .gt. 0) then
               call mpi_recv(xq_id,nquad,mpi_double_precision,id,2,MPI_COMM_WORLD,istatus,ierr)
               call mpi_recv(yq_id,nquad,mpi_double_precision,id,3,MPI_COMM_WORLD,istatus,ierr)
               call mpi_recv(zq_id,nquad,mpi_double_precision,id,4,MPI_COMM_WORLD,istatus,ierr)
               call mpi_recv(wq_id,nquad,mpi_double_precision,id,5,MPI_COMM_WORLD,istatus,ierr)
             endif
             call hdf5_group_open(file_id, "MPIGroup", group_id)
             write (unit=s,fmt="(i0)") id
             call hdf5_write(file_id, group_id, "xold_proc_"//trim(s), x_id)
             if (nloc .gt. 0) then
                 call hdf5_write(file_id, group_id, "nquad_proc_"//trim(s), nquad)
                 call hdf5_write(file_id, group_id, "xq_proc_"//trim(s), xq_id(1:nquad))
                 call hdf5_write(file_id, group_id, "yq_proc_"//trim(s), yq_id(1:nquad))
                 call hdf5_write(file_id, group_id, "zq_proc_"//trim(s), zq_id(1:nquad))
                 call hdf5_write(file_id, group_id, "wq_proc_"//trim(s), wq_id(1:nquad))
             endif
             call hdf5_group_close(group_id)
          enddo
        endif
