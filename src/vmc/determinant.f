 ...
 10   continue
      do 400 iab=1,2
         ...
         do j=1,nel
            ...
            jk=jk+nel
            call dcopy(nel,orb(1+ish,jorb,kstate),1,slmi(1+jk,iab),1)
            call dcopy(nel,dorb(1,1+ish,jorb,kstate),3,fp(1,j,iab),nel*3)
            call dcopy(nel,dorb(2,1+ish,jorb,kstate),3,fp(2,j,iab),nel*3)
            call dcopy(nel,dorb(3,1+ish,jorb,kstate),3,fp(3,j,iab),nel*3)
            call dcopy(nel,ddorb(1+ish,jorb,kstate),1,fpp(j,iab),nel)
         enddo
...
c     compute kinetic contribution of B+Btilde to compute Eloc
      ...
      do i=1,nelec
         do iorb=1,norb+nadorb
            b(iorb,i)=-hb*(ddorb(i,iorb,kstate)+2*(vj(1,i)*dorb(1,i,iorb,kstate)
     &            +vj(2,i)*dorb(2,i,iorb,kstate)+vj(3,i)*dorb(3,i,iorb,kstate)))
         enddo
      enddo
...
c     compute derivative of kinetic contribution of B+Btilde wrt jastrow parameters
      ...
      if(ioptjas.gt.0) then
         do iparm=1,nparmj
            do i=1,nelec
               do iorb=1,norb
                  b_dj(iorb,i,iparm)=-2*hb*(g(1,i,iparm)*dorb(1,i,iorb,kstate)
     &            +g(2,i,iparm)*dorb(2,i,iorb,kstate)+g(3,i,iparm)*dorb(3,i,iorb,kstate))
               enddo
            enddo
         enddo
      endif
         ...
         do ic=1,ncent
            do i=1,nelec
               do l=1,3
                  ...
                  call daxpy(norb,2*da_vj(l,1,i,ic),dorb(1,i,1,kstate),3*MELEC,b_da(l,i,1,ic),3*MELEC)
                  call daxpy(norb,2*da_vj(l,2,i,ic),dorb(2,i,1,kstate),3*MELEC,b_da(l,i,1,ic),3*MELEC)
                  call daxpy(norb,2*da_vj(l,3,i,ic),dorb(3,i,1,kstate),3*MELEC,b_da(l,i,1,ic),3*MELEC)
                  do iorb=1,norb
