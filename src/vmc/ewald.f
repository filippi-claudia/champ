...
c      write(ounit,*) "cutr",cutr,deltar
      arg=(1.0d0+deltar)
c      write(ounit,*) "arg",arg(1)
...
c      write(ounit,'(/,''Shortest distance to sim. cell boundary'',f14.8)') dist_min/2
>>>>>>> main:src/vmc/ewald.f
...
c      write(ounit,*) "parameters for separate"
c      write(ounit,*) 'ngnorm_big',ngnorm_big
c      write(ounit,*) 'igmult',igmult
c      write(ounit,*) "gnorm",gnorm
c      write(ounit,*) "ngnorm",ngnorm
c      write(ounit,*) "cutr",cutr
c      write(ounit,*) "ncoef_per",ncoef_per
...
c     e-e Jastrow
c      write(ounit,*) "nloc before computing e-ion local part", nloc 
>>>>>>> main:src/vmc/ewald.f
...
c      write(ounit,*) "end separate 00005"
...
!       write(ounit,'(''r,vpseudo,z*derf/r,z/r'',9d12.4)') r(ir),vpseudo(ir,ict,lpot(ict))+znuc(ict)/r(ir),
!    & -znuc(ict)*derf(alpha*r(ir))/r(ir)+znuc(ict)/r(ir),vpseudo(ir,ict,lpot(ict))+znuc(ict)*derf(alpha*r(ir))/r(ir)

        vpseudo(ir,ict,lpot(ict))=vps_short(ir)
      enddo

! Derivative at origin 0 because of nature of psp, and at last pt 0 because we subtracted out asymp behaviour
      dpot1=0
      dpotn=0
...
! 100   write(ounit,'(''CHECK SR'',i5,g12.5,9d12.4)') ir,r(ir),vps_short(ir)
...
! 110   write(ounit,'(''CHECK FT'',i5,g12.5,d12.4)') ig,gnorm(ig),vbare_psp(ig)
...
!     vbare_psp(1)=vbare_psp(1)+pi*znuc(ict)/(vcell*alpha**2)
      vbare_psp(1)=vps_basis_fourier(1)+pi*znuc(ict)/(vcell*alpha**2)
      do ig=2,ngnorm_big
        g2=gnorm(ig)**2
        g2a=0.25d0*g2/alpha**2
!       write(ounit,*) ig,twopi,znuc(ict),exp(-g2a),(vcell*g2)
! 120   vbare_psp(ig)=vbare_psp(ig)-2*twopi*znuc(ict)*exp(-g2a)/(vcell*g2)
...
c        print*, 'cutr',cutr,'dx',dx
c        write(ounit,'(''start loop vsrange1'')')
c        write(ounit,'(''npts'',i20)') npts
        do i=1,npts
          r_tmp(1)=(i-1)*dx+1.d-20
          r_tmp(2)=0.d0
          r_tmp(3)=0.d0
...
c     decreasing number of images by 1 temporally reagarding the ao's implementation at basis_fns
         n_images=n_images-1
