...
! vector of wave function parameters
...
! historically, we input -deltap in compute_parameters, so we multiply actual deltap by -1
        ...
        if(iflag.ne.0) then
          ...
          stop
!         sr_adiag=10*sr_adiag
!         write(ounit,'(''sr_adiag increased to '',f10.5)') sr_adiag
!         go to 6
        endif
...
!         call check_length_run_sr(iter,inc_nblk,nblk,nblk_max,denergy,denergy_err,energy_err_sav,energy_tol)
