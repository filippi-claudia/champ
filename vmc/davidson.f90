...
contains
    ...
    subroutine generalized_eigensolver_dense(mtx, eigenvalues, ritz_vectors, lowest, method, max_iters, &
        tolerance, iters, max_dim_sub, stx, idtask)
    ...
    integer, intent(in) :: idtask, lowest
...
contains
 ...
 subroutine generalized_eigensolver_free(fun_mtx_gemv, eigenvalues, ritz_vectors, nparm, lowest, method, max_iters, &
       tolerance, iters, max_dim_sub, fun_stx_gemv, idtask)
    ...
    ! input/output variable
    integer, intent(in) :: nparm, lowest, idtask
    real(dp), dimension(lowest), intent(out) :: eigenvalues
    real(dp), dimension(:, :), intent(out) :: ritz_vectors
    integer, intent(in) :: max_iters
    integer, intent(in), optional :: max_dim_sub
    ...
    interface
       ...
       function fun_mtx_gemv(nparm, lowest, input_vect) result(output_vect)
       ...
       function fun_stx_gemv(nparm, lowest, input_vect) result(output_vect)
    ...
    diag_mtx = extract_diagonal_free(fun_mtx_gemv, nparm, lowest, dim_mtx)
    diag_stx = extract_diagonal_free(fun_stx_gemv, nparm, lowest, dim_mtx)
    ...
    mtx_proj = lapack_matmul('T', 'N', V, fun_mtx_gemv(nparm, lowest, V))
    stx_proj = lapack_matmul('T', 'N', V, fun_stx_gemv(nparm, lowest, V))
    ...
    outer_loop: do i=1, max_iters
       ...
       ritz_vectors = lapack_matmul('N', 'N', V, eigenvectors_sub(:, :lowest))
       do j=1,lowest
          guess = eigenvalues_sub(j) * fun_stx_gemv(nparm, lowest, reshape(ritz_vectors(:, j),(/dim_mtx,1/) ) )
          rs = fun_mtx_gemv(nparm, lowest, reshape(ritz_vectors(:, j), (/dim_mtx,1/))) - guess
      ...
      ! ENDIF IDTASK
       ...
       if (size(V, 2) <= max_dim) then
          ...
          correction = compute_DPR_free(fun_mtx_gemv, fun_stx_gemv, nparm, lowest, V, &
          ...
          ! 8. Update the the projection 
          call update_projection_free(fun_mtx_gemv, nparm, lowest, V, mtx_proj)
          call update_projection_free(fun_stx_gemv, nparm, lowest, V, stx_proj)
  ...
  function compute_DPR_free(fun_mtx_gemv, fun_stx_gemv, nparm, lowest,  V, eigenvalues, eigenvectors, &
    diag_mtx, diag_stx) result(correction)
      ...
      correction = fun_mtx_gemv(nparm, lowest, vectors) - lapack_matmul( &
      'N','N',fun_stx_gemv(nparm, lowest, vectors), diag_eigenvalues)
 ...
 function extract_diagonal_free(fun_A_gemv, nparm, lowest, dim) result(out)
   ...
   interface
      function fun_A_gemv(nparm, lowest, input_vect) result(output_vect)
        !> \brief Function to compute the optional mtx on the fly
        !> \param[in] i column/row to compute from mtx
        !> \param vec column/row from mtx
        use numeric_kinds, only: dp
   ...
   do ii = 1,nparm
      tmp_array = 0E0
      tmp_array(ii,1) = 1.0
      tmp_array = fun_A_gemv(nparm, 1, tmp_array)
 ...
 subroutine update_projection_free(fun_A_gemv, nparm, lowest, V, A_proj)
   !> \brief update the projected matrices
   !> \param A: full matrix
   !> \param V: projector
   !> \param A_proj: projected matrix
   ...
   allocate(tmp_array(nvec, nvec))
   tmp_array(:old_dim, :old_dim) = A_proj
   tmp_array(:,old_dim+1:) = lapack_matmul('T', 'N', V, fun_A_gemv(nparm, lowest, V(:, old_dim+1:)))
   tmp_array( old_dim+1:,:old_dim ) = transpose(tmp_array(:old_dim, old_dim+1:))
