...
c e-e terms
        ...
        fsn(i,j)=deriv_psibnl(rij,dpsij_ratio(iparm0+1,iq),isb,ipar)
...
c e-e-n terms
        do ic=1,ncent
          ...
          if(nparmc(it).gt.0) then
            iparm0=npoint(it)
            fsn(i,j)=fsn(i,j) +
     &      deriv_psinl(rij,r_en_quad(iq,ic),r_en(jj,ic),dpsij_ratio(iparm0+1,iq),it)
          endif
        enddo
...
c e-n terms
   47 fsn(iel,iel)=0
     ...
     &                 deriv_psianl(r_en_quad(iq,ic),dpsij_ratio(iparm0+1,iq),it)
      enddo
      do it=1,nctype
        iparm0=npointa(it)
      ...
      if(isc.ge.12) call scale_dist(rij,u,3)
        ...
        do ic=1,ncent
          ...
          if(nparmc(it).gt.0) then
            iparm0=npoint(it)
            fsn(i,j)=fsn(i,j) +
     &      deriv_psinl(u,rr_en2_quad(ic),rr_en2(jj,ic),dpsij_ratio(iparm0+1,iq),it)
          endif
        enddo
