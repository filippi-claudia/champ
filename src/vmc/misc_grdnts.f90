module misc_grdnts
! -----------------------------------------------------------------------
!   Various subroutines used when energy gradients for atoms are
!   calculated.
!
...
!   Subroutine which at the start up prints out information about the
!   energy gradients (cartesian).
...
!   Subroutine which at the start up prints out information about the
!   energy gradients (z matrix/internal).
...
!   Subroutine which calculates and printouts energy gradients
!   for cartesian coordinates of atoms from energy differences
!   calculated using correlated smapling.
...
!   Subroutine which calculates and printouts energy gradients
!   for Z matrix coordinates of atoms from energy differences
!   calculated using correlated smapling.
...
!   Subroutine which calculates the displacement for energy gradients
!   using Z matrix (internal) coordinates
      ...
      use zmatrix, only: czcart,czcart_ref,czint,izcmat
...
!   Subroutine which prints out at the start of a run
!   information regarding the Z matrix.
...
!   Subroutine which calculates and print outs the diagonal
!   part of the Hessian for Z matrix coordinates of atoms
!   from energy differences  calculated using correlated smapling.
...
! -----------------------------------------------------------------------
      ...
      use zmatrix, only: czcart_ref,czint,izcmat
...
!     write(ounit,*) 'HELLO'
!     do 20 ic=1,ncent
! 20    write(ounit,'(3i5)') (izcmat(k,ic),k=1,3)

!     call cart2zmat(MCENT,czcart,izcmat,czint)
!     write(ounit,*) 'HELLO'
!     do 20 ic=1,ncent
! 20    write(ounit,'(1p3e12.5)') (czint(k,ic),k=1,3)
!     call zmat2cart(MCENT,izcmat,czint,czcart)

!     write(ounit,*) 'HELLO'
!     do 21 ic=1,ncent
! 21    write(ounit,'(1p3e12.5)') (czcart(k,ic),k=1,3)
...
!     return
      ...
      ncent3=3*ncent
!     do ic=1,ncent_ind
!       write(ounit,*) 'Bmat',(bwilson(ic,jc),jc=1,ncent3)
!     enddo
