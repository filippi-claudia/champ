...
! Exercise population control if dmc or vmc with weights
          ...
          if(nforce.gt.1.and.idrifdifgfunc.gt.0) then
            ...
            dfus=0.5d0*dfus/tau
            drifdifgfunc=-dfus
!           drifdifgfunc=drifdifgfunc-2*log(rnorm_nodes)
          endif
...
!         if(idrifdifgfunc.eq.0)wtnow=wtnow/rnorm_nodes**2
...
!           write(ounit,*) 'IN DMC',ajacold(iw,ifr)
            ro=1.d0
            ...
            wtg_derivsum1=wtg(1)
!           if(idrifdifgfunc.eq.0)then
!             wtg=wt(iw)*fprod/rnorm_nodes**2
!             wtg_derivsum1=wtg
!            else
!             wtg=wt(iw)*fprod
!             wtg_derivsum1=wtg/rnorm_nodes**2
!           endif
        ...
        enddo
!       write(ounit,*) 'IN DMC',ajacold(iw,2)

!       wtg=wt(iw)*fprod/rnorm_nodes**2
!       write(*,*)'prima ',wtg,eold(iw,2),pwt(iw,2),ajacold(iw,2),psido_dmc(iw,2),psijo_dmc(iw,2),idrifdifgfunc
!       call deriv(wtg,eold(iw,1),pwt(iw,1),ajacold(iw,1),psido_dmc(iw,1),psijo_dmc(iw,1),idrifdifgfunc)
!       call deriv(wtg,eold,pwt,ajacold,psido_dmc,psijo_dmc,idrifdifgfunc,iw,mwalk)
